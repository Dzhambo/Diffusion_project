...
def get_activations(images, batch_size, device):
    ...
    inception_network = to_device(inception_network, device)
...
def fid_score(images1, images2, device, batch_size=100):
    up = torch.nn.Upsample(size=(299, 299), mode="bilinear")
    mu1, sigma1 = calculate_activation_statistics(up(images1), batch_size, device)
    mu2, sigma2 = calculate_activation_statistics(up(images2), batch_size, device)
